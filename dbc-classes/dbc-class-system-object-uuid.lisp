...
(defgeneric system-identity-parent-uuid (sys-object)
  (:method  ((sys-object system-object-uuid))
    (when (slot-boundp sys-object 'system-identity-parent-uuid)
      (slot-value sys-object 'system-identity-parent-uuid)))
  (:documentation "Return the base-namespace UUID for SYS-OBJECT'S system-identity-parent-uuid slot-value."))
...
(defgeneric (setf system-identity-parent-uuid) (uuid sys-object)
  (:method  ((uuid unicly:unique-universal-identifier) (sys-object system-object-uuid))
    (setf (slot-value sys-object 'system-identity-parent-uuid)
          uuid))
  (:documentation "Set UUID as SYS-OBJECT's system-identity-parent-uuid slot-value."))

(defgeneric system-identity-uuid-byte-array (sys-object)
  (:method ((sys-object system-object-uuid))
    (when (slot-boundp sys-object 'system-identity-uuid-byte-array)
      (slot-value  sys-object 'system-identity-uuid-byte-array)))
  (:documentation "Accessor for SYS-OBJECT's system-identity-uuid-byte-array slot-value."))
...
(defgeneric (setf system-identity-uuid-byte-array) (byte-array sys-object)
  (:method ((byte-array array) (sys-object system-object-uuid))
    ...
    (setf (slot-value  sys-object 'system-identity-uuid-byte-array)
          byte-array))
  (:documentation "Set SYS-OBJECT's UUID namespace byte-array representation with BYTE-ARRAY.
...
(defgeneric (setf system-identity-uuid-bit-vector) (bv sys-object)
  ...
  (:documentation "Set SYS-OBJECT's UUID namespace bit-vector representation with BV.
...
(defgeneric (setf system-identity-uuid-integer) (uuid-integer-128 sys-object)
  (:method  ((uuid-integer-128 bignum) (sys-object system-object-uuid))
    ...
    (setf (slot-value sys-object 'system-identity-uuid-integer)
          uuid-integer-128))
  (:documentation "Set SYS-OBJECT's 128bit integer representation with UUID-INTEGER-128."))
...
(defgeneric (setf system-identity-uuid-string-36) (uuid-hex-string-36 sys-object)
  (:method ((uuid-hex-string-36 string) (sys-object system-object-uuid))
    ...
    (setf (slot-value sys-object 'system-identity-uuid-string-36)
          uuid-hex-string-36))
  (:documentation "Set SYS-OBJECT's `unicly::uuid-hex-string-36' representation with UUID-HEX-STRING-36."))  
...
(defgeneric system-identity (sys-object)
  (:method((sys-object system-object-uuid))
    (when (slot-boundp sys-object 'system-identity)
      (slot-value sys-object 'system-identity)))
  (:documentation "Accessor for SYS-OBJECT's system-identity slot-value."))
...
(defgeneric (setf system-identity) (namespace-and-identity sys-object)
  ...
  (:method :around
    ...
    (destructuring-bind (namespace identity) namespace-and-identity ;; (list (make-v4-uuid)  "<IDENTITY>")
      ...
      (let ((new-namespace       (unicly:make-v5-uuid namespace (if (symbolp identity) 
                                                                    (string identity)
                                                                    identity)))
            (old-id-slot         (system-identity                 sys-object))
            (old-base-namespace  (system-identity-parent-uuid     sys-object))
            ;; We might not have any slots set or only some, so get all of them.
            (old-namespace-slot  (system-identity-uuid            sys-object))
            (old-byte-array-slot (system-identity-uuid-byte-array sys-object))
            (old-bit-vector-slot (system-identity-uuid-bit-vector sys-object))
            (old-integer-slot    (system-identity-uuid-integer    sys-object))
            (old-hex-string-slot (system-identity-uuid-string-36  sys-object))
  ...
  (:documentation "Set SYS-OBJECT's system-identity slot-value to IDENTITY"))
...
(defgeneric system-identity-uuid (sys-object)
  ...
  (:documentation "Accessor for SYS-OBJECT's system-identity-uuid slot-value."))
...
(defgeneric (setf system-identity-uuid) (coercable-uuid sys-object)
  ...
  (:method  :after 
    ...
    (let* ((uuid-bv-prevent (unicly:uuid-to-bit-vector 
      ...
      (if (system-identity sys-object)
          (if (system-identity-parent-uuid sys-object)
  ...
  (:documentation "Set uuid namespace for SYS-OBJECT with COERCABLE-UUID
...
(defmethod system-object-uuid-description ((sys-object system-object-uuid) &key stream verbose)
  ...
  (if (not verbose)
      (let* ((unbound "#<UNBOUND>")
             (sys-id-if  (system-identity sys-object))
             (sys-id     (if sys-id-if
                             (prin1-to-string sys-id-if)
                             unbound))
             (sys-id-uuid   (or (system-identity-uuid sys-object)
                                unbound)))
      ...
      (let* ((unbound "#<UNBOUND>")
             ...
             (sys-id-uuid   (or (system-identity-uuid sys-object)
                                unbound))
             (byte-array    (or (system-identity-uuid-byte-array sys-object)
                                unbound))
             (bit-vector    (or (system-identity-uuid-bit-vector sys-object)
                                unbound))
             (hex-string-if (system-identity-uuid-string-36 sys-object))
             (hex-string    (if hex-string-if
                                (prin1-to-string hex-string-if)
                                unbound))
             (integer-128-if (system-identity-uuid-integer sys-object))
             (integer-128    (if integer-128-if
                                 (let ((*print-base* 16)
                                       (*print-radix* t)) 
                                   (princ-to-string integer-128-if))
                                 unbound))
             (parent-uuid    (or (system-identity-parent-uuid sys-object)
                                 unbound))         
             (version-if    (system-identity-uuid-version sys-object))
